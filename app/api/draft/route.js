...
export async function POST(request) {
  try {
    ...
    const { dateKey, openingQtys, closingQtys, expectedSales, closerName, savedToday } = body;

    await sql`
      INSERT INTO till_drafts
        (date_key, opening_qtys, closing_qtys, expected_sales, closer_name, saved_today, updated_at)
      VALUES
        (${dateKey}, ${JSON.stringify(openingQtys)}, ${JSON.stringify(closingQtys)},
         ${expectedSales || 0}, ${closerName || ""}, ${savedToday || false}, NOW())
